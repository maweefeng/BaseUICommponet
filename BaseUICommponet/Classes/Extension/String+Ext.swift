...
extension String {
    ...
    func stringToDate(_ dateFormat:String = "yyyy-MM-dd HH:mm:ss") -> Date? {
    ...
    static func generateRandomJPEGString(header: String,type:String) -> String {
        let knumber = 32
        let source_string = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ"
        var resultStr = ""
        ...
        let dateStr = Date().dateToString("yyyy-MM-dd")
        let cosPath = "\(header)/" +  dateStr + "/" +  resultStr + ".\(type)"
        return cosPath
        
    }
    
    static func generateRandomWavString(header: String,interval:String) -> String {
        let knumber = 32
        let source_string = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ"
        var resultStr = ""
    ...
    static func changeSecondToChineseString(_ seconds: Int) -> String{
        let time = seconds
        if seconds < 60{
    ...
    static func getDurationString(_ header: String) -> String{
        let suffix = header.components(separatedBy: "-").last
        let seconds = suffix?.components(separatedBy: ".").first
        let intSeconds = Int(seconds ?? "0")
    ...
    static func getWavTimevalString(_ header: String) -> String{
    ...
    ///   - font: 字体
    /// - Returns: 富文本
    func changePartOfStringStyle(_ rangeString:String, color:UIColor, font:UIFont? = UIFont.systemFont(ofSize: 14))->NSMutableAttributedString
    {
    ...
    func insertImageGerenateAttributeString(imageName:String,color:UIColor, font:UIFont? = UIFont.systemFont(ofSize: 14)) ->NSMutableAttributedString{
    ...
    func changePartOfStringStyleWithLine(_ rangeString:String, color:UIColor, font:UIFont)->NSMutableAttributedString
    {
        let attriStr:NSMutableAttributedString = NSMutableAttributedString(string:self)
        let range = NSMakeRange(NSString(string: self).range(of: rangeString).location, NSString(string: self).range(of: rangeString).length)
    ...
    func changePartOfStringStyleWithDeleteLine(_ rangeString:String, color:UIColor, font:UIFont)->NSMutableAttributedString
    {
        let attriStr:NSMutableAttributedString = NSMutableAttributedString(string:self)
        let range = NSMakeRange(NSString(string: self).range(of: rangeString).location, NSString(string: self).range(of: rangeString).length)
